#include "ValueSettingsInputHandler.hpp"
...
void ValueSettingsInputHandler::beforeBlockUse(BeforeBlockUseEvent &ev)
{
    ...
    if (isClientSide) {
        CreateClient::VALUE_SETTINGS_HANDLER.cancelIfWarmupAlreadyStarted(ev);
    }
    ...
    for (auto& [_, behaviourShared] : sbe->getAllBehaviours()) {
        ...
        if (isClientSide) {
            ...
            CreateClient::VALUE_SETTINGS_HANDLER.startInteractionWith(pos, type, ray.mFacing);
        }
    }
}