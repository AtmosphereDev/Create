...
Vec3 BeltHelper::getVectorForOffset(BeltBlockEntity *controller, float offset)
{
    ...
    int verticality = slope == BeltSlope::DOWNWARD ? -1 : slope == BeltSlope::UPWARD ? 1 : 0;
    float verticalMovement = (offset < 0.5f) ? 0.0f : verticality;
    verticalMovement = verticalMovement * (std::min(offset, static_cast<float>(controller->beltLength) - 0.5f) - 0.5f);
    Vec3 vec = Vec3::atLowerCornerOf(Vec3(controller->getBlockPos())) ;
